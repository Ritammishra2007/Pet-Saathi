    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: C.bg, fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ background: C.surface, padding: "20px 20px 16px", borderBottom: `1px solid ${C.grayLight}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{ background: C.navy, color: "white", border: "none", borderRadius: "12px", padding: "10px 16px", fontSize: "13px", fontWeight: 700, cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>+ Add Pet</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 24px" }}>
        ...
        {/* Add pet card */}
        <button onClick={() => setShowAdd(true)} style={{ width: "100%", background: C.surface, border: `1.5px dashed ${C.orange}`, borderRadius: "18px", padding: "20px", display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", cursor: "pointer", fontFamily: "'Nunito', sans-serif" }}>
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: C.surface, borderRadius: "24px 24px 0 0", padding: "24px 20px 36px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              <div>
                ...
                <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Mochi, Luna" style={{ width: "100%", padding: "13px 16px", borderRadius: "14px", border: `1.5px solid ${C.grayLight}`, fontSize: "15px", fontWeight: 600, color: C.navy, fontFamily: "'Nunito', sans-serif", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: "12px", fontWeight: 800, color: C.gray, display: "block", marginBottom: "6px" }}>BREED (OPTIONAL)</label>
                <input value={newBreed} onChange={e => setNewBreed(e.target.value)} placeholder="e.g. Persian, Labrador" style={{ width: "100%", padding: "13px 16px", borderRadius: "14px", border: `1.5px solid ${C.grayLight}`, fontSize: "15px", fontWeight: 600, color: C.navy, fontFamily: "'Nunito', sans-serif", outline: "none", boxSizing: "border-box" }} />
              </div>
              <button onClick={() => setShowAdd(false)} disabled={!newName} style={{ width: "100%", padding: "15px", borderRadius: "14px", background: newName ? C.navy : C.grayLight, border: "none", color: newName ? "white" : C.gray, fontSize: "15px", fontWeight: 800, cursor: newName ? "pointer" : "default", fontFamily: "'Nunito', sans-serif", marginTop: "4px" }}>
                Continue to Setup →
              </button>
            </div>
          </div>
